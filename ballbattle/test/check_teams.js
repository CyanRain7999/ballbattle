...
(async () => {
  ...
  for (const [id, n] of [['grid3', 5], ['ring', 8], ['slalom', 5], ['random', 6], ['randomsym', 6]]) {
  ...
  check('对称随机布局左右镜像', await evalJS(`(() => {
    ...
    return l.length === 6 && l.every((o, i) => i % 2 === 0 ? Math.abs((1 - o.fx - o.fw) - l[i + 1].fx) < 1e-9 && Math.abs(o.fy - l[i + 1].fy) < 1e-9 && Math.abs(o.fw - l[i + 1].fw) < 1e-9 : true);
  })()`));
  ...
  check('BOSS 属性（1800 血/62 半径/techx+curse）', await evalJS(`(() => {
    const b = battle.orbs[4];
    return b.maxHp === 1800 && b.r === 62 && b.ability === 'techx' && b.skill2 === 'curse' && b.maxCd2 > 0;
  ...
  await evalJS(`(() => {
    ...
    check('组合战斗正常推进', r && r.t > 25 && r.teams && r.ob === 6 && r.shrinkLv >= 1, JSON.stringify(r));
